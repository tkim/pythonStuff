...
def brighten(image, factor):
    ...
    new_im.arry = image.array * factor
...
def blur(image, kernel_size):
    ...
    x_pixels, y_pixels, num_channels = image.array.share #represents x, y pixels of image, # channels (R, G, B)
    new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_channels) # mkaing a new array to copy values to!
...
def apply_kernel(image, kernel):
    ...
    x_pixels, y_pixels, num_channels = image.array.shape # represents x, y pixels of image, # channels (R, G, B)
    new_im = Image(x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_channels) # making a new array to copy values to!
    neighbor_range = kernal.shape[0] // 2 # this is a variable that tells us how many neighbors we actually look at (ie for a 3x3 kernal, this value should be 1)
    for x in range(x_pixels):
        for y in range(y_pixels):
            for c in range(num_channels):
                total = 0
...
if __name__ == '__main__':
    ...
    incu_contrast.write_image('increased_contrast.png')
    ...
    blur_15.write_image('blur_k5.png')
